...
**CI:** **[`phase1-smoke.yml`](.github/workflows/phase1-smoke.yml)** runs stdlib **`unittest`** on **[`tests/test_eval_report_routing.py`](tests/test_eval_report_routing.py)** (path formatting + **`load_routing_from_eval_report`**) **before** installing torch, then the smoke matrix and the same routing gates below. **[`phase3-smoke.yml`](.github/workflows/phase3-smoke.yml)** and Phase 1 both run **`python scripts/routing_policy.py --from-checkpoint …`** on the smoke-trained checkpoint so **`eval_report.json`** must still contain the Phase 2 **`routing`** object on every PR (same check you can run locally after any train).
...
| **CI regression** | **[`phase1-smoke.yml`](.github/workflows/phase1-smoke.yml)** runs **`unittest`** on **`tests/`** first, then **`horizon1_route_then_retrieve.py --verify`** and **`routing_policy.py --from-checkpoint`** on **`artifacts/phase1/runs/smoke/ag_news/scratch`** after the smoke matrix. **[`phase3-smoke.yml`](.github/workflows/phase3-smoke.yml)** runs the same glue + **`routing_policy`** pair on **`.tmp/phase3-smoke`** after the tiny train (before ONNX). |
