...
| Layer | What you get |
| --- | --- |
| **Generation** | Causal **instruct** LM for chat and tool-composed answers. **Default in code:** `HuggingFaceTB/SmolLM2-360M-Instruct` (override with Space env **`HORIZON2_MODEL`** or local **`--model`**). **`--lm-only`** turns off encoder, FAQ RAG, and memory but keeps generation + slash commands. |
| **TinyModel1 encoder** | **4-label topic classification** (AG News–style: World, Business, Sports, Sci/Tech) as a **soft hint** in system context and in the optional *Brain trace*; **`/classify`** returns the full probability table. **Dense embeddings** power **`/similarity`**, **`/embed`**, **`/nearest`**, and hybrid **FAQ retrieval**. |
| **FAQ RAG** | Bundled corpus (`texts/rag_faq_corpus.md` in-repo; copied into the Space artifact). **Hybrid** retrieval (keyword overlap + encoder similarity). Toggle FAQ injection and **FAQ grounding** strictness via chat phrases (see `/help`). |
| **Memory** | **Horizon 3** SQLite: **long-term** and **session** notes scoped by **`scope_key`**; list/export/forget from chat; **`Start a new private session`** / **`Switch to scope …`** for demo isolation (not a substitute for real auth). |
| **Routing** | **JSON intent router** maps plain language to tools: chat, summarize, reformulate, grounded Q&A, FAQ retrieve, **web_search**, classify, similarity, embedding, nearest, remember/list/clear, status. **`--no-smart-route`** or *Turn off smart routing* → chat + slash shortcuts only. |
| **Web (optional)** | When Hub secrets **`GOOGLE_CSE_API_KEY`** and **`GOOGLE_CSE_CX`** are set: **`/web`**, routed **`web_search`**, and an optional **auto-web** upgrade for time-sensitive wording (disable with **`NO_AUTO_WEB=1`** or **`--no-auto-web`**). Snippets are injected into context; the model cites **[Web n]** when using them. |
| **NL controls** | Short phrases (no slash) adjust **session** scope, toggles, and **reply-style** hints (`nl_controls.parse_control_action`). |
| **Embedded prompt signals** | Long “normal chat” lines can **imply** one-turn overlays: tradeoffs (**pros/cons vs narrative**), **tables prefer vs avoid**, **numbered vs continuous steps**, reply language, **code-only vs code with explanation**, **pseudocode vs runnable code**, **fixed option count (options_n=N)**, **diagram vs no diagram**, **risks-first vs benefits-first order**, **revise/polish draft**, **topic guardrails (don’t mention X)**, **STAR / PREP / IRAC answer frames**, length caps, **verbosity brief vs detailed**, guided discovery / **full solution (not hints)**, red-team, **supportive coaching**, ephemeral, a11y, beginner audience, **technical audience**, register, JSON / **plain text (no JSON)**, strict speculation, **creative brainstorming**, BLUF / direct answer opening, runnable commands, **conceptual-only (no commands)**, **bullets vs prose**, assumptions / **decisive tone**, example density, exposition order, follow-up closing, clarify-first, section headings, analogies, **bold key terms vs minimal bold**, **acronym spell-out vs terse**, **risk posture (safe vs pragmatic)**, **FAQ quote vs paraphrase**, **emoji include vs avoid**, **FAQ grounding strict vs relaxed**, **cite sources vs no links**, **ranked options / priority order**, **checklist vs no checkboxes**, **math show-work vs final-only**, **code fences vs inline** — see **`prompt_signals:`** in the *Brain trace* and the Space UI section **Testing embedded prompt signals**. |
| **Limits** | Small models: answers may be **wrong or shallow** on hard reasoning; RAG/web **reduce** but do not **eliminate** hallucinations. **Not multimodal** by default. **Public Space** demo memory may share a default scope with other visitors unless you start a **private session**. |
...
- **Live web & other search backends** — The **Space** supports **Google Programmable Search (Custom Search JSON API)** when **`GOOGLE_CSE_API_KEY`** + **`GOOGLE_CSE_CX`** are configured (see the generated Space `README.md` from [`scripts/build_space_artifact.py`](scripts/build_space_artifact.py) and **Google web search** in the Gradio instructions under the chat). For a broader comparison of vendor APIs, self-hosted options, quotas, and policy notes, see [`texts/plan-internet-access-for-universal-brain.md`](texts/plan-internet-access-for-universal-brain.md) (section **4**).
- **Space project (Hub)** — [HyperlinksSpace/TinyModel1Space](https://huggingface.co/spaces/HyperlinksSpace/TinyModel1Space): deployed **Universal Brain chat** (Gradio: NL routing, generative replies, FAQ RAG, memory, classifier); source layout is generated by [`scripts/build_space_artifact.py`](scripts/build_space_artifact.py) in CI. **`python scripts/build_space_artifact.py -h`** lists **`--namespace`**, **`--version`**, **`--output-dir`**, optional **`--model-id`**, and copy-paste examples (**stdlib-only**; **`tests/test_build_space_artifact_help.py`** covers **`-h`**).
- **Space: natural-language session controls (no slash commands)** — In the chat box you can type plain requests; a small deterministic matcher runs **before** the LLM JSON router so these actions stay reliable **without slash commands**. **Embedded prompt signals** (normal chat, after that pass) read trade-off (**pros/cons vs narrative**) / **tables prefer vs avoid** / how-to / **numbered vs continuous steps** / language / **code-only** / **code with explanation** / **pseudocode vs runnable code** / **exactly N options** / **diagram vs no diagram** / **risks-first vs benefits-first** / **revise draft** / **topic guardrails** / **STAR-PREP-IRAC frames** / **length-cap** / **verbosity brief vs detailed** / **guided (hints-first)** / **full solution (not hints)** / **red-team critique** / **supportive coaching** / **ephemeral (no memory)** / **a11y (screen reader / WCAG)** / **beginner (ELI5-style)** / **technical (expert-depth)** / **register (board vs Slack tone)** / **JSON in prose** / **plain text only in prose** / **strict facts (no guessing)** / **creative brainstorming** / **BLUF / summary-first** / **direct answer (no TL;DR)** / **runnable shell commands** / **conceptual-only (no commands)** / **bullets vs prose** / **assumptions & limitations (caveats)** / **decisive confident tone** / **concrete vs example-free** / **definitions-first vs intuition-first** / **minimal vs suggest closing** / **clarify-first vs answer-first** / **markdown headings vs flat prose** / **analogy vs literal-only** / **bold key terms vs minimal bold** / **acronym spell-out vs terse** / **risk posture (safe vs pragmatic)** / **FAQ quote vs paraphrase** / **emoji include vs avoid** / **FAQ grounding strict vs relaxed** / **cite sources vs no links** / **ranked options / priority order** / **checklist vs no checkboxes** / **pseudocode vs runnable code** / **exactly N options** / **diagram vs no diagram** / **risks-first vs benefits-first** / **revise draft** / **topic guardrails** / **STAR-PREP-IRAC frames** / **math show-work vs final-only** / **code fences vs inline** wording and adjust **this turn**—check the brain trace for **`prompt_signals:`**; step-by-step tests live in the Space UI under **Testing embedded prompt signals**. **Try on the Space:** **`What is my current scope?`** / **`Show my session settings`** (prints `scope_key` + toggle state), **`Start a new private session`** (generates a fresh `ub-…` scope so memory is isolated from the default demo namespace), **`Switch to scope my-key`** / **`Use session team-a`** (set an ASCII `scope_key` from plain language). **Reply shape (assistant chat replies):** short lines like **`Be brief`**, **`More detail please`**, **`Normal answer length`**, **`Use bullet points`** / **`Prefer numbered lists`**, **`No bullets, plain paragraphs`**, **`Reset reply style`** (injects soft instructions into the system context; avoids long questions accidentally flipping modes). **FAQ grounding:** **`Strict FAQ`** / **`FAQ only`** / **`Stick to the FAQ`** (treat policy/process claims as supported only when clearly in the supplied FAQ excerpts; cite **`[FAQ excerpt N]`**), **`Balanced FAQ`** / **`Normal FAQ`** / **`Default FAQ grounding`**, **`Relaxed FAQ`** / **`FAQ plus general knowledge`**. **`Reset reply style`** also restores defaults for FAQ grounding, **audience** depth, **answer opening**, **procedure steps**, **confidence tone**, **follow-up closing**, **concept order**, **examples**, **comparisons**, **register**, **code layout**, **analogies**, **acronym style**, **clarify-first**, **speculation level**, **math detail**, **output format**, **risk posture**, **actionability**, **quote style**, **markdown tables**, **emoji**, **section headings**, **term emphasis**, and **counterpoints**. **Quote style:** **`Quote the FAQ excerpts`** / **`Use direct quotes`** / **`Cite with quotes`** vs **`Paraphrase only`** / **`Don't quote excerpts`** / **`No quotes`** vs **`Default quote style`** / **`Normal quote style`**. **Tables:** **`Use tables`** / **`Markdown tables`** / **`Tabular format`** vs **`No tables`** / **`Avoid tables`** / **`No markdown tables`** vs **`Default table style`** / **`Normal tables`**. **Emoji:** **`Use emoji`** / **`Emoji ok`** / **`Include emoji`** / **`Add emoji`** vs **`No emoji`** / **`Avoid emoji`** / **`Don't use emoji`** vs **`Default emoji style`** / **`Normal emoji`**. **Section headings:** **`Use section headings`** / **`Organize with headings`** / **`Use markdown headings`** vs **`No section headings`** / **`Avoid markdown headings`** / **`Flat answer`** / **`Flat prose`** vs **`Default section headings`** / **`Normal headings`**. **Term emphasis:** **`Bold key terms`** / **`Highlight important terms`** / **`Emphasize keywords`** vs **`Minimal bold`** / **`Don't overuse bold`** / **`Avoid excessive bold`** vs **`Default emphasis`** / **`Normal bold`**. **Counterpoints:** **`Challenge my assumptions`** / **`Play devils advocate`** / **`Push back on weak points`** vs **`Be supportive`** / **`Assume good intent`** / **`Encourage my ideas`** vs **`Default counterpoints`** / **`Normal pushback`**. **Risk posture:** **`Be risk averse`** / **`Be conservative`** / **`Err on the side of safety`** vs **`Be pragmatic`** / **`Optimize for speed`** / **`Good enough is fine`** vs **`Default risk posture`**. **Actionability:** **`Give me runnable commands`** / **`Include commands`** / **`Make it actionable`** vs **`No commands`** / **`Conceptual only`** / **`High level only`** vs **`Default actionability`**. **Math detail:** **`Show your work`** / **`Show the derivation`** / **`Include steps in math`** vs **`Final answer only`** / **`No derivation`** / **`Skip the steps`** vs **`Default math detail`**. **Output format:** **`Answer in JSON`** / **`JSON output`** vs **`Plain text only`** / **`No JSON`** / **`No structured output`** vs **`Default output format`**. **Clarify-first:** **`Ask clarifying questions first`** / **`Clarify first`** vs **`No clarifying questions`** / **`Just answer without questions`** vs **`Default clarify mode`**. **Speculation:** **`No speculation`** / **`Stick to high confidence only`** / **`Avoid guessing`** vs **`Brainstorm freely`** / **`Speculate freely`** / **`Wild ideas ok`** vs **`Default speculation`**. **Audience (explanation depth):** **`Explain simply`**, **`ELI5`**, **`I'm a beginner`**, **`Assume I'm new`** / **`I need the basics`** vs **`Expert mode`**, **`Assume I'm technical`** / **`Technical audience`** vs **`Normal explanation level`**. **Answer opening:** **`TLDR first`**, **`Lead with a summary`**, **`Summary first`** vs **`No TLDR`**, **`Skip the summary`**, **`Answer directly`** vs **`Default answer structure`**. **Procedure layout:** **`Step by step`**, **`Step-by-step`**, **`Numbered steps`**, **`Walk me through the steps`**, **`Break it into steps`** vs **`No numbered steps`**, **`Don't number steps`**, **`Continuous prose`** / **`Prose without steps`** vs **`Default step style`**. **Confidence tone:** **`Flag your assumptions`**, **`Be explicit about uncertainty`**, **`Say if you don't know`**, **`Tell me when you're unsure`** vs **`Be decisive`**, **`Don't hedge`**, **`Give firm answers`** vs **`Reset uncertainty`**. **Follow-up ending:** **`Suggest next steps`**, **`Offer follow-up questions`**, **`End with next steps`** vs **`No follow-up questions`**, **`Don't ask follow-up questions`**, **`No questions at the end`** vs **`Default follow-ups`**. **Concept order:** **`Definitions first`**, **`Start with definitions`**, **`Define terms first`** vs **`Intuition first`**, **`Big picture first`**, **`Start with the big picture`** vs **`Default explanation order`**. **Examples:** **`Include examples`**, **`Use concrete examples`**, **`Illustrate with examples`** vs **`Skip examples`**, **`Don't add examples`**, **`No examples unless I ask`** vs **`Default examples`**. **Comparison layout:** **`Use pros and cons`**, **`Pros and cons sections`**, **`Compare with pros and cons`** vs **`Compare in flowing prose`**, **`Prose comparison only`**, **`No pros and cons sections`** vs **`Default comparison style`**. **Register:** **`Formal tone`**, **`Professional register`**, **`Business writing style`** vs **`Casual tone`**, **`Speak casually`**, **`Friendly casual style`** vs **`Default tone`**, **`Neutral tone`**. **Code blocks:** **`Use code fences`**, **`Fenced code blocks`**, **`Markdown code fences`** vs **`Inline code only`**, **`No fenced code blocks`**, **`No triple backticks`** vs **`Default code formatting`**, **`Reset code style`**. **Analogies:** **`Use analogies`**, **`Analogies when helpful`**, **`Metaphors are ok`** vs **`No analogies`**, **`Skip metaphors`**, **`Literal explanations only`** vs **`Default analogy style`**. **Acronyms:** **`Spell out acronyms`**, **`Expand acronyms on first use`**, **`Define acronyms when you use them`** vs **`Assume I know acronyms`**, **`Don't expand acronyms`**, **`Keep acronyms as is`** vs **`Default acronym style`**. Also: **`Export my memories`** (Horizon 3 JSON export), **`Download my notes`**, **`Show my memories`**, **`Clear my session notes`**, **`Delete all my memories for this chat`** (forget-scope for that scope), **`Turn off FAQ context`** / **`Turn FAQ back on`**, **`Turn off smart routing`** / **`Turn smart routing back on`**, **`Show the brain trace`** / **`Hide the brain trace`**. **`/help`** in the app summarizes the same phrases. **Privacy note:** the deployed Space defaults to a shared **`--memory-scope`** unless changed at deploy time — use **`Start a new private session`** (or **`Switch to scope …`**) to move your chat onto a distinct key for demos; production still needs proper auth/tenant design.
- **Space / chat output triage** — if live behavior looks off (flat classifier, slow generations, weak FAQ hits), use [`texts/model-output-improvement-guide.md`](texts/model-output-improvement-guide.md) (symptom → which script or flag) and the sample backlog [`texts/universal-brain-session-improvement-plan.txt`](texts/universal-brain-session-improvement-plan.txt).
...
**`python scripts/routing_policy.py -h`** repeats these examples in the help epilog (same paths as above).
...
**CI:** **[`phase1-smoke.yml`](.github/workflows/phase1-smoke.yml)** and **[`phase3-smoke.yml`](.github/workflows/phase3-smoke.yml)** each call the shared composite action **[`stdlib-unittest`](.github/actions/stdlib-unittest/action.yml)** (stdlib **`unittest`** on **`tests/`**, including **[`tests/test_eval_report_routing.py`](tests/test_eval_report_routing.py)** and **[`tests/test_rag_faq_smoke_help.py`](tests/test_rag_faq_smoke_help.py)** for **`rag_faq_smoke.py -h`** and **[`tests/test_nl_controls.py`](tests/test_nl_controls.py)** for deterministic **Space NL control** phrases), **before** installing torch / Phase 3 extras. The same composite is the **first** step in **Horizon** PR / **`main`** smokes **[`horizon2-smoke.yml`](.github/workflows/horizon2-smoke.yml)**, **[`horizon3-smoke.yml`](.github/workflows/horizon3-smoke.yml)**, **[`horizon4-smoke.yml`](.github/workflows/horizon4-smoke.yml)**, **[`horizon6-smoke.yml`](.github/workflows/horizon6-smoke.yml)**, **[`horizon7-smoke.yml`](.github/workflows/horizon7-smoke.yml)**, **[`horizon8-smoke.yml`](.github/workflows/horizon8-smoke.yml)**, and **[`horizon9-smoke.yml`](.github/workflows/horizon9-smoke.yml)** (before their pip installs or each workflow’s **`--verify`**). **[`deploy-hf-space-versioned.yml`](.github/workflows/deploy-hf-space-versioned.yml)** runs it before **`pip install huggingface_hub`** and building the Space artifact. **[`parity-hub-local-smoke.yml`](.github/workflows/parity-hub-local-smoke.yml)** trains a tiny local checkpoint and validates **`parity_check_hub_vs_local.py`** JSON (`hub-vs-local.json`) against `HyperlinksSpace/TinyModel1`. Phase 1 then runs the smoke matrix and routing gates; Phase 3 runs a tiny train, the same **`routing_policy.py --from-checkpoint …`** check on **`.tmp/phase3-smoke`**, ONNX, parity, and bench. In both Phase workflows **`eval_report.json`** must still contain the Phase 2 **`routing`** object on every PR (same check you can run locally after any train).
...
| **Verify A CLI help** | `python scripts/horizon1_verify_short_term_a.py -h` | Epilog: full run vs **`--skip-phase3`** and the **phase1_compare → train → Phase 3** chain; **`tests/test_horizon1_verify_short_term_a_help.py`**. |
...
| **Three-task CLI help** | `python scripts/horizon1_three_datasets.py -h` | Epilog: default run, **`--offline-datasets`**, **`--output-root`**; **`tests/test_horizon1_three_datasets_help.py`**. |
...
| **`eval_report_routing.py`** | Loads **`routing`** from **`eval_report.json`** (`load_routing_from_eval_report`), optional banner (`maybe_print_routing_section`), and shared copy-paste helpers (`format_routing_policy_from_checkpoint_command`, `print_routing_policy_from_checkpoint_tip`) for training and matrix summaries; used by **`rag_faq_smoke`**, **`embeddings_smoke_test`**, **`horizon1_route_then_retrieve`**, **`routing_policy.py --from-checkpoint`**, **`train_tinymodel1_classifier`**, **`finetune_pretrained_classifier`**, **`phase1_compare`**, **`horizon1_three_datasets`**, and **`horizon1_verify_short_term_a`**. Regressions: **`tests/test_eval_report_routing.py`** (tip helpers, **`load_routing_from_eval_report`** incl. non-directory path and empty **`routing`** dict, **`maybe_print_routing_section`**) and **`tests/test_routing_policy_cli.py`** (**`routing_policy.py -h`** epilog); locally: **`python -m unittest discover -s tests -p "test_*.py" -v`**. |
| **`rag_faq_smoke.py`** | FAQ chunking, hybrid scores, cheap keyword overlap; **`--query`** for one-off citation-style traces; **`--show-train-routing`** prints Phase 2 **`routing`** from **`eval_report.json`** before retrieval (shared **`eval_report_routing`**). **`python scripts/rag_faq_smoke.py -h`** lists copy-paste examples. **`TinyModelRuntime`** loads only when retrieval runs (**`main()`**); **`tests/test_rag_faq_smoke_help.py`** covers **`-h`** in stdlib-only **`unittest`**. |
...
| **`embeddings_smoke_test.py`** | **`--routing`** (and optional **`--min-confidence`** / **`--min-margin`**) prints **`RoutingDecision`** next to classifier top‑k; **`--show-train-routing`** prints **`eval_report.json`**’s **`routing`** block first (shared helper **`scripts/eval_report_routing.py`** with **`horizon1_route_then_retrieve`**). **`python scripts/embeddings_smoke_test.py -h`** lists copy-paste examples (loads **`torch`**; not in stdlib-only **`tests/`**). |
| **CI regression** | **[`phase1-smoke.yml`](.github/workflows/phase1-smoke.yml)** runs **[`stdlib-unittest`](.github/actions/stdlib-unittest/action.yml)** then **`horizon1_route_then_retrieve.py --verify`** and **`routing_policy.py --from-checkpoint`** on **`artifacts/phase1/runs/smoke/ag_news/scratch`** after the smoke matrix. **[`phase3-smoke.yml`](.github/workflows/phase3-smoke.yml)** runs the same composite action, then the glue + **`routing_policy`** pair on **`.tmp/phase3-smoke`** after the tiny train (before ONNX). |
...
2. **FAQ retrieval:** `python scripts/rag_faq_smoke.py --model <dir>` or `python scripts/rag_faq_smoke.py --query "How do I get a refund?" --top-k 3 --model <dir>` (optional **`--show-train-routing`**). Same flows are summarized under **`python scripts/rag_faq_smoke.py -h`**.
...
5. **Classifier + gates without corpus:** `python scripts/embeddings_smoke_test.py --model <dir> --routing` (add **`--show-train-routing`** to echo Phase 2 **`routing`** like step 4). Examples also appear under **`python scripts/embeddings_smoke_test.py -h`**.
...
[`scripts/embeddings_smoke_test.py`](https://github.com/HyperlinksSpace/TinyModel/blob/main/scripts/embeddings_smoke_test.py) runs [`TinyModelRuntime`](https://github.com/HyperlinksSpace/TinyModel/blob/main/scripts/tinymodel_runtime.py) on a few queries: **classification probabilities**, **pairwise similarity**, and **retrieval** over a toy candidate list (support/triage scenario). Run **`python scripts/embeddings_smoke_test.py -h`** for copy-paste **`--model`**, **`--routing`**, and **`--show-train-routing`** examples.
...
**`python scripts/parity_check_hub_vs_local.py -h`** prints the same multi-line example as **`plan.txt`** (train once to **`artifacts/parity-smoke`**, then compare).
...
On success, **`finetune_pretrained_classifier.py`** prints the same **`routing_policy.py --from-checkpoint`** **Tip:** for **`--output-dir`** as **`train_tinymodel1_classifier.py`**.
...
| **PR smoke: Hub vs local parity** (tiny local train → parity JSON vs Hub model) | [`parity-hub-local-smoke.yml`](https://github.com/HyperlinksSpace/TinyModel/blob/main/.github/workflows/parity-hub-local-smoke.yml) |
...
- **[`deploy-hf-space-versioned.yml`](https://github.com/HyperlinksSpace/TinyModel/blob/main/.github/workflows/deploy-hf-space-versioned.yml)** — Runs **[`stdlib-unittest`](.github/actions/stdlib-unittest/action.yml)** first (includes **`tests/test_build_space_artifact_help.py`** for **`build_space_artifact.py -h`**), then bundles the **Universal Brain chat** UI (`scripts/universal_brain_chat.py` + dependencies) via `scripts/build_space_artifact.py` and uploads **`{namespace}/TinyModel{version}Space`**.  
  - **Secrets:** `HF_TOKEN`.  
