...
Maintainer deployment policy: [`texts/HUGGING_FACE_DEPLOYMENT_INTERNAL.md`](texts/HUGGING_FACE_DEPLOYMENT_INTERNAL.md). Capability reference for the Space: [`texts/universal-brain-capabilities.md`](texts/universal-brain-capabilities.md). Research synthesis (text-in / text-out stack): [`texts/tinymodel-text-in-text-out-research.md`](texts/tinymodel-text-in-text-out-research.md). Universal Brain concept vs TinyModel today: [`texts/universal-brain-concept-vs-tinymodel-today.md`](texts/universal-brain-concept-vs-tinymodel-today.md). Forward plan (universal capability, tokens, self-development): [`texts/universal-brain-forward-plan-self-development.md`](texts/universal-brain-forward-plan-self-development.md). Golden prompts + eval runner: [`texts/golden-prompts/README.md`](texts/golden-prompts/README.md) · `python scripts/ub_eval_runner.py --verify`. Hyperlinks Space Program integration: [`plan/README.md`](plan/README.md) (master plan) · [`texts/hsp-tinymodel-integration-strategy.md`](texts/hsp-tinymodel-integration-strategy.md) · `python scripts/hsp_rag_smoke.py --verify` · `python scripts/hsp_rag_hybrid_smoke.py --verify` (torch) · `python scripts/hsp_route_then_retrieve.py --verify` (torch) · `python scripts/hsp_corpus_export.py --verify` · `python scripts/hsp_integration_smoke.py --verify` · `python scripts/hsp_phase3_contract_smoke.py --verify` · `python scripts/hsp_phase3_server_smoke.py --verify` (live HTTP API).
...
| **Embedded prompt signals** | Long “normal chat” lines can **imply** one-turn overlays: tradeoffs (**pros/cons vs narrative**), **tables prefer vs avoid**, **numbered vs continuous steps**, reply language, **code-only vs code with explanation**, **pseudocode vs runnable code**, **fixed option count (options_n=N)**, **diagram vs no diagram**, **risks-first vs benefits-first order**, **risks and mitigations (paired)**, **revise/polish draft**, **email format (compose)**, **formal letter format**, **press release (media announcement)**, **release notes (changelog)**, **operational runbook format**, **job aid (quick reference)**, **meeting agenda (timeboxed)**, **revise with before/after diff**, **topic guardrails (don’t mention X)**, **required topics (must cover)**, **glossary / define key terms**, **UK vs US spelling**, **chronological / reverse-chronological timeline**, **blameless postmortem format**, **sprint retrospective (retro)**, **user story (As a / I want / So that)**, **definition of done (DoD)**, **5 Whys root-cause analysis**, **fishbone / Ishikawa cause-and-effect diagram**, **second vs third person voice**, **FAQ Q&A (Q:/A:) pairs**, **STAR / PREP / IRAC answer frames**, length caps, **verbosity brief vs detailed**, guided discovery / **full solution (not hints)**, red-team, **supportive coaching**, ephemeral, a11y, beginner audience, **technical audience**, register, JSON / **plain text (no JSON)**, strict speculation, **creative brainstorming**, BLUF / direct answer opening, **recommendation before rationale**, **go/no-go gate verdict**, **summary at end (closing recap)**, **decision matrix (criteria × options)**, **build vs buy (in-house vs vendor)**, **one-pager executive brief**, **status report (periodic update)**, **action plan (owners & due dates)**, **RACI role-assignment matrix**, **stakeholder map (influence & interest)**, **SWOT analysis**, **PESTLE macro-environment analysis**, **cost-benefit analysis**, **open questions / TBD section**, **best/base/worst case scenarios**, runnable commands, **conceptual-only (no commands)**, **bullets vs prose**, assumptions / **decisive tone**, example density, exposition order, follow-up closing, clarify-first, section headings, analogies, **bold key terms vs minimal bold**, **acronym spell-out vs terse**, **risk posture (safe vs pragmatic)**, **FAQ quote vs paraphrase**, **emoji include vs avoid**, **FAQ grounding strict vs relaxed**, **cite sources vs no links**, **ranked options / priority order**, **checklist vs no checkboxes**, **math show-work vs final-only**, **code fences vs inline** — see **`prompt_signals:`** in the *Brain trace* and the Space UI section **Testing embedded prompt signals**. |
...
1. Open the [live app](https://hyperlinksspace-tinymodel1space.hf.space) (or the [Hub Space](https://huggingface.co/spaces/HyperlinksSpace/TinyModel1Space)).
2. Type in **plain language** or use **`/help`** for slash commands.
3. Say **Show the brain trace** to see routing hints and the footer line **`prompt_signals:`** on replies.
4. Scroll below the chat to **Testing embedded prompt signals** for copy-paste examples of **embedded** (one-turn) style detection.

Maintainer reference (same feature set, grouped): [`texts/universal-brain-capabilities.md`](texts/universal-brain-capabilities.md).

#### Slash commands and routed tools

| Task | Slash shortcut |
| --- | --- |
| Help / session status | `/help`, `/status` |
| Summarize / rephrase | `/summarize`, `/reformulate` |
| Q&A with pasted facts | `/grounded` (`question \|\|\| context`) |
| FAQ hybrid search | `/retrieve` |
| Web search (needs secrets) | `/web` |
| Classify / embed / similarity / nearest | `/classify`, `/embed`, `/similarity`, `/nearest` |
| Memory | `/remember`, `/session`, `/memories`, `/clear-session` |

Natural-language routing covers the same intents when **smart routing** is on.

#### Session control phrases (short; persist until reset)

Examples (no slash): **What is my current scope?** · **Start a new private session** · **Switch to scope my-key** · **Show the brain trace** · **Turn off FAQ context** · **Be brief** / **More detail please** · **Use bullet points** · **Strict FAQ** · **ELI5** / **Expert mode** · **Reset reply style** (restores defaults). Full phrase list: type **`/help`** in the chat or see [`texts/universal-brain-capabilities.md`](texts/universal-brain-capabilities.md).

#### Embedded prompt signals (long chat; one turn)

Wording inside a normal question can set reply shape for **that answer only** (e.g. *pros and cons*, *answer in Spanish*, *code only*, *hints only*, *rank these options*, *decision matrix with criteria as rows*, *build vs buy*, *one-pager format*, *weekly status report format*, *action plan with owners and due dates*, *RACI matrix*, *stakeholder map*, *SWOT analysis*, *PESTLE analysis*, *cost-benefit analysis*, *go/no-go decision*, *open questions section*, *best case base case worst case*, *lead with your recommendation*, *risks and mitigations*, *blameless postmortem format*, *sprint retrospective format*, *user story format*, *definition of done*, *five whys analysis*, *fishbone diagram*, *Mermaid diagram*, *write an email to the client*, *formal business letter format*, *press release format*, *release notes format*, *runbook format*, *job aid / quick reference card*, *meeting agenda with timeboxes*, *revise my draft*, *before and after*, *include a glossary of key terms*, *use British English spelling*, *chronological timeline*, *address the reader as you*, *Q&A format for the FAQ*, *wrap up with a summary at the end*, *don’t mention competitors*, *must cover security and SLA*, *STAR format*). Check **`prompt_signals:`** in the brain trace. **Step-by-step table:** **Testing embedded prompt signals** in the Space UI (synced from `scripts/universal_brain_chat.py`).
...
- **Space project (Hub)** — [HyperlinksSpace/TinyModel1Space](https://huggingface.co/spaces/HyperlinksSpace/TinyModel1Space): deployed **Universal Brain chat** (Gradio: NL routing, generative replies, FAQ RAG, memory, classifier). Artifact layout is built by [`scripts/build_space_artifact.py`](scripts/build_space_artifact.py) in CI; the generated Space **`README.md`** is the Hugging Face **Space card description**. **`python scripts/build_space_artifact.py -h`** lists **`--namespace`**, **`--version`**, **`--output-dir`**, optional **`--model-id`**. Capability overview: sections above and [`texts/universal-brain-capabilities.md`](texts/universal-brain-capabilities.md); in-app **`/help`** and **Testing embedded prompt signals** under the chat.
