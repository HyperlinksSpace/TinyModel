...
- **Space: natural-language session controls (no slash commands)** — In the chat box you can type plain requests; a small deterministic matcher runs **before** the LLM router and handles **governance / UX toggles**. **Try on the Space:** say **`Export my memories`** (JSON export for the current SQLite scope), **`Download my notes`**, **`Show my memories`**, **`Clear my session notes`**, **`Delete all my memories for this chat`** (**forget-scope** wipe for that scope), **`Turn off FAQ context`** / **`Turn FAQ back on`** (stops injecting FAQ excerpts into the system prompt), **`Turn off smart routing`** / **`Turn smart routing back on`** (skips the JSON intent router; normal chat + classifier/RAG context still run), **`Show the brain trace`** / **`Hide the brain trace`** (toggles the optional *Brain trace* footer). **`/help`** in the app repeats the same phrases. **Privacy note:** the demo Space uses a shared default SQLite scope (`ub-chat-default` unless you redeploy with another **`--memory-scope`**), so treat exports/forget actions as **demo-grade** unless you assign per-user scopes yourself.
