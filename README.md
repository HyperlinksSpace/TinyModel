...
| **Embedded prompt signals** | Long “normal chat” lines can **imply** one-turn overlays: tradeoffs, tables, steps, reply language, code-only, length caps, guided discovery, red-team, **supportive coaching**, ephemeral, a11y, beginner audience, **technical audience**, register, JSON, strict speculation, BLUF, runnable commands, assumptions, example density, exposition order, follow-up closing, clarify-first, section headings, analogies, **bold key terms vs minimal bold**, **acronym spell-out vs terse**, **risk posture (safe vs pragmatic)**, **FAQ quote vs paraphrase**, **emoji include vs avoid**, **FAQ grounding strict vs relaxed**, **math show-work vs final-only**, **code fences vs inline** — see **`prompt_signals:`** in the *Brain trace* and the Space UI section **Testing embedded prompt signals**. |
