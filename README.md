...
Wording inside a normal question can set reply shape for **that answer only** (e.g. *pros and cons*, *answer in Spanish*, *code only*, *hints only*, *rank these options*, *Mermaid diagram*, *revise my draft*, *don’t mention competitors*, *STAR format*). Check **`prompt_signals:`** in the brain trace. **Step-by-step table:** **Testing embedded prompt signals** in the Space UI (synced from `scripts/universal_brain_chat.py`).
