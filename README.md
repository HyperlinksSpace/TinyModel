...
Wording inside a normal question can set reply shape for **that answer only** (e.g. *pros and cons*, *answer in Spanish*, *code only*, *hints only*, *rank these options*, *Mermaid diagram*, *revise my draft*, *before and after*, *include a glossary of key terms*, *don’t mention competitors*, *must cover security and SLA*, *STAR format*). Check **`prompt_signals:`** in the brain trace. **Step-by-step table:** **Testing embedded prompt signals** in the Space UI (synced from `scripts/universal_brain_chat.py`).
