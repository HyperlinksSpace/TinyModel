...
| **`eval_report_routing.py`** | Loads **`routing`** from **`eval_report.json`** (`load_routing_from_eval_report`), optional banner (`maybe_print_routing_section`), and shared copy-paste helpers (`format_routing_policy_from_checkpoint_command`, `print_routing_policy_from_checkpoint_tip`) for training and matrix summaries; used by **`rag_faq_smoke`**, **`embeddings_smoke_test`**, **`horizon1_route_then_retrieve`**, **`routing_policy.py --from-checkpoint`**, **`train_tinymodel1_classifier`**, **`finetune_pretrained_classifier`**, **`phase1_compare`**, **`horizon1_three_datasets`**, and **`horizon1_verify_short_term_a`**. Regressions: **`tests/test_eval_report_routing.py`** (tip helpers, **`load_routing_from_eval_report`** incl. non-directory path and empty **`routing`** dict, **`maybe_print_routing_section`**) — locally: **`python -m unittest discover -s tests -p "test_*.py" -v`**. |
