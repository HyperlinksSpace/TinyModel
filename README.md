...
Wording inside a normal question can set reply shape for **that answer only** (e.g. *pros and cons*, *answer in Spanish*, *code only*, *hints only*, *rank these options*, *decision matrix with criteria as rows*, *Mermaid diagram*, *revise my draft*, *before and after*, *include a glossary of key terms*, *use British English spelling*, *chronological timeline*, *address the reader as you*, *Q&A format for the FAQ*, *wrap up with a summary at the end*, *don’t mention competitors*, *must cover security and SLA*, *STAR format*). Check **`prompt_signals:`** in the brain trace. **Step-by-step table:** **Testing embedded prompt signals** in the Space UI (synced from `scripts/universal_brain_chat.py`).
