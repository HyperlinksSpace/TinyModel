...
| **This file** | **Vision + staged capabilities** toward a unified “brain-like” stack (Horizons **0–77**): through **H36–H37** freezes and pair cardinality; **H38–H39** **monotonic checkpoints** and **mutually exclusive job** scheduling; **H40–H41** **composite policy AND** and **geo-fence / residency** allow-lists; **H42–H43** **egress URL allow-lists** and **credential max-age** ceilings; **H44–H45** **optimistic concurrency** revisions and **payload size** ceilings; **H46–H47** **latency p99 budgets** and **global kill-switch** overrides; **H48–H49** **dual-control approvals** and **pinned artifact digests**; **H50–H51** **wire-format major-version compatibility** and **storage utilization headroom**; **H52–H53** **RBAC role-subset gates** and **dry-run mutation blocks**; **H54–H55** **backup recency ceilings** and **sensitive-tier encryption requirements**; **H56–H57** **TLS protocol version allow-lists** for ingress and **severity→pager routing** for incidents; **H58–H59** **open advisory budgets** vs **critical/high ceilings** and **release-channel signature gates**; **H60–H61** **SPDX license allow-lists** for dependency policy and **maintainer quorum** floors for ownership; **H62–H63** **protected-branch merge gates** (approvals + CI) and **secret-scan finding ceilings**; **H64–H65** **container image age ceilings** (supply-chain freshness) and **post-incident RCA documentation deadlines**; **H66–H67** **deprecated dependency ceilings** (tech-debt) and **DSAR-style export turnaround SLAs** by customer tier; **H68–H69** **blocking static-analysis finding ceilings** (quality bars) and **vendor API hostname allow-lists** (subprocessor egress); **H70–H71** **major-incident backlog ceilings** (ops hygiene) and **payment overdue grace windows** (subscription continuity); **H72–H73** **P1 support-ticket backlog ceilings** and **vendor contract renewal-notice/runway checks**; **H74–H75** **open critical penetration-test finding ceilings** and **disaster-recovery drill recency windows**; **H76–H77** **WCAG blocker ceilings** for accessibility gates and **legal-hold mutation locks**—then product layers beyond this repo. |
...
### Horizon 52 — **RBAC subset gate (required ⊆ granted)**

**Goal:** coarse **role bundles** attached to principals must **cover** operation requirements—encoded as **set inclusion**: **`required_roles ⊆ granted_roles`** before destructive APIs run (**H9**, admin consoles).

**Exit criteria**

- **Attribute-based access** (**ABAC**), **delegation graphs**, **JIT elevation** with expiry—not only static strings.

**Implemented in this repository (MVP):** `texts/horizon52_role_subset_sample.json` + `scripts/horizon52_role_subset_smoke.py` — `--verify` marks **`allow`** iff **`required_roles`** as a set is **subset of** **`granted_roles`**; writes `horizon52_role_subset_run/1.0` under `.tmp/horizon52-role-subset/run.json`. **Not done yet vs. full exit:** **wildcards**, **hierarchical roles**, **break-glass** scopes.

---

### Horizon 53 — **Dry-run gate (simulation blocks mutations)**

**Goal:** **`dry_run`** tooling pathways must never apply **mutating side effects**—operators rehearse promotions safely beside **H24** canaries (**Terraform plan**, CI **`--dry-run`** modes).

**Exit criteria**

- **Shadow environments**, **traffic replay**, **contract tests** against prod-shaped configs—not only boolean flags.

**Implemented in this repository (MVP):** `texts/horizon53_dry_run_gate_sample.json` + `scripts/horizon53_dry_run_gate_smoke.py` — `--verify` marks **`allow`** iff it is **not** the case that **both** **`dry_run`** and **`mutating_operation`** are true; writes `horizon53_dry_run_gate_run/1.0` under `.tmp/horizon53-dry-run-gate/run.json`. **Not done yet vs. full exit:** **partial dry-run**, **mixed read/write batches**, **two-phase commit** rehearsals.

---

### Horizon 54 — **Backup recency (RPO-style age ceiling)**

**Goal:** disaster-recovery drills depend on **fresh restore points**—encode **`backup_age_hours`** against **`max_allowed_age_hours`** so stale snapshots fail promotion (**H36** freezes intersect DR rehearsals).

**Exit criteria**

- **Immutable backups**, **cross-region replication**, **restore time objectives** proven by game days—not only JSON thresholds.

**Implemented in this repository (MVP):** `texts/horizon54_backup_recency_sample.json` + `scripts/horizon54_backup_recency_smoke.py` — `--verify` marks **`compliant`** iff **`backup_age_hours ≤ max_allowed_age_hours`** per check; writes `horizon54_backup_recency_run/1.0` under `.tmp/horizon54-backup-recency/run.json`. **Not done yet vs. full exit:** **continuous replication lag**, **checksum proofs**, **legal hold** carve-outs.

---

### Horizon 55 — **Encryption required (sensitive tier)**

**Goal:** labeled **sensitive** datasets (**PII**, **credentials**) must never land on volumes without **encryption at rest**—pair **H35** crypto suites with **storage tier** classification.

**Exit criteria**

- **Customer-managed keys**, **envelope encryption**, **access logs** per CM—not only booleans.

**Implemented in this repository (MVP):** `texts/horizon55_encryption_required_sample.json` + `scripts/horizon55_encryption_required_smoke.py` — `--verify` marks **`allow`** iff **non-sensitive tier** **or** **encryption at rest enabled** per check; writes `horizon55_encryption_required_run/1.0` under `.tmp/horizon55-encryption-required/run.json`. **Not done yet vs. full exit:** **field-level** crypto, **HSM** bindings, **tenant KMS** isolation.

---

### Horizon 56 — **TLS version allow-list (ingress handshake)**

**Goal:** terminate TLS only on **approved protocol versions**—reject legacy **`tls1.0`/`tls1.1`** clients before cipher negotiation (**H34–H35** crypto complements protocol posture).

**Exit criteria**

- **Cipher suite policies**, **OCSP stapling**, **certificate transparency** monitors—not only version strings.

**Implemented in this repository (MVP):** `texts/horizon56_tls_version_sample.json` + `scripts/horizon56_tls_version_smoke.py` — `--verify` marks **`allow`** iff **`offered_tls_version`** is in **`allowed_tls_versions`** (case-insensitive); writes `horizon56_tls_version_run/1.0` under `.tmp/horizon56-tls-version/run.json`. **Not done yet vs. full exit:** **real** handshake probes, **per-listener** matrices, **automated** downgrade detection.

---

### Horizon 57 — **Severity routing (pager required)**

**Goal:** incidents classified **`sev1`**/**`critical`** must **page on-call**—no silent escalation gaps beside **Decision gates** ops discipline.

**Exit criteria**

- **Rotation calendars**, **customer comms**, **post-incident** blameless reviews—not only booleans.

**Implemented in this repository (MVP):** `texts/horizon57_sev_pager_sample.json` + `scripts/horizon57_sev_pager_smoke.py` — `--verify` marks **`compliant`** iff **severity not in** **`severities_requiring_page`** **or** **`pager_sent`** is true; writes `horizon57_sev_pager_run/1.0` under `.tmp/horizon57-sev-pager/run.json`. **Not done yet vs. full exit:** **multi-channel** paging, **SLO burn** bridges, **customer-visible** status hooks.

---

### Horizon 58 — **Vulnerability budget (critical/high ceilings)**

**Goal:** shipping pipelines block when **open advisory counts** breach policy—pair **H29** SBOM discipline with **measurable** debt ceilings before prod promotion.

**Exit criteria**

- **Scanner quorum**, **reachability analysis**, **timeboxed waivers** with owners—not only integer thresholds.

**Implemented in this repository (MVP):** `texts/horizon58_vuln_budget_sample.json` + `scripts/horizon58_vuln_budget_smoke.py` — `--verify` marks **`compliant`** iff **`critical_open ≤ max_critical_open`** and **`high_open ≤ max_high_open`** per check; writes `horizon58_vuln_budget_run/1.0` under `.tmp/horizon58-vuln-budget/run.json`. **Not done yet vs. full exit:** **CVE age** decay, **ephemeral** vs **persistent** vulns, **vendor SLAs**.

---

### Horizon 59 — **Signature gate (release channels)**

**Goal:** **`prod`**/**`release`** artifacts must carry **valid signatures**—unsigned drops fail closed beside **H49** digest pinning.

**Exit criteria**

- **Sigstore/rekor** transparency, **key ceremonies**, **hardware-backed** signing—not only booleans.

**Implemented in this repository (MVP):** `texts/horizon59_signature_gate_sample.json` + `scripts/horizon59_signature_gate_smoke.py` — **`allow`** iff **channel not in** **`channels_requiring_signature`** **or** **`signature_valid`** is true; writes `horizon59_signature_gate_run/1.0` under `.tmp/horizon59-signature-gate/run.json`. **Not done yet vs. full exit:** **threshold** signatures, **OIDC** workload identity, **artifact attestations**.

---

### Horizon 60 — **SPDX license allow-list (supply-chain policy)**

**Goal:** promoted builds fail when any dependency carries a **license id** outside the org **allow-list**—pair **H58** advisory posture with **legal/compliance** hygiene.

**Exit criteria**

- **OR/SExpression license parsing**, **distribution graphs**, **exception** workflows—not only string equality.

**Implemented in this repository (MVP):** `texts/horizon60_license_allow_sample.json` + `scripts/horizon60_license_allow_smoke.py` — **`compliant`** iff **`dependency_license`** (normalized) is in **`allowed_license_ids`**; writes `horizon60_license_allow_run/1.0` under `.tmp/horizon60-license-allow/run.json`. **Not done yet vs. full exit:** **composite** SPDX expressions, **propagation** across transitive closure, **counsel sign-off** queues.

---

### Horizon 61 — **Maintainer quorum (bus-factor floor)**

**Goal:** critical repositories retain **≥ N distinct maintainers** so incidents and releases do not hinge on one human—pairs **Decision gates** operations discipline.

**Exit criteria**

- **CODEOWNER coverage**, **succession attestations**, **time-off handoffs**—not only counting integers.

**Implemented in this repository (MVP):** `texts/horizon61_maintainer_quorum_sample.json` + `scripts/horizon61_maintainer_quorum_smoke.py` — **`compliant`** iff **`maintainer_count ≥ min_maintainers`** per check; writes `horizon61_maintainer_quorum_run/1.0` under `.tmp/horizon61-maintainer-quorum/run.json`. **Not done yet vs. full exit:** **verified identity** mappings, **region diversity**, **ownership drift** alerts.

---

### Horizon 62 — **Protected-branch merge gate (approvals + CI)**

**Goal:** **`main`**/**`release`** (and similar) require **minimum approvals** **and** **green CI** before merge—pair **H52–H53** governance with SCM mechanics.

**Exit criteria**

- **Merge queues**, **merge trains**, **required status contexts** beyond booleans—not only scripted checks.

**Implemented in this repository (MVP):** `texts/horizon62_branch_protect_sample.json` + `scripts/horizon62_branch_protect_smoke.py` — **`allow_merge`** iff **branch not protected** **or** (**`min_approvals_met`** **and** **`ci_green`**); writes `horizon62_branch_protect_run/1.0` under `.tmp/horizon62-branch-protect/run.json`. **Not done yet vs. full exit:** **branch-specific** policies, **stacked PR** semantics, **bot identity** carve-outs.

---

### Horizon 63 — **Secret-scan sweep ceiling**

**Goal:** pipelines block promotion while **open secret-detector findings** exceed **`max_open_secret_findings`**—pairs **H58–H59** supply-chain hygiene with leak prevention.

**Exit criteria**

- **Entropy scoring**, **revocation drills**, **blast-radius maps** linked to scanners—not only counters.

**Implemented in this repository (MVP):** `texts/horizon63_secret_sweep_sample.json` + `scripts/horizon63_secret_sweep_smoke.py` — **`compliant`** iff **`open_secret_findings ≤ max_open_secret_findings`** per check; writes `horizon63_secret_sweep_run/1.0` under `.tmp/horizon63-secret-sweep/run.json`. **Not done yet vs. full exit:** **false-positive** workflows with expiry, **custom detectors**, **historical scrub** attestations.

---

### Horizon 64 — **Container image age ceiling (freshness)**

**Goal:** deployments reject **stale base layers**—encoded **`image_age_days`** against **`max_image_age_days`** so rotting images fail beside **H58–H59** hygiene (**SBOM**, signatures).

**Exit criteria**

- **Rolling rebuild** schedules, **minimal attack surfaces**, **digest-only** pulls—not only age integers.

**Implemented in this repository (MVP):** `texts/horizon64_image_age_sample.json` + `scripts/horizon64_image_age_smoke.py` — **`compliant`** iff **`image_age_days ≤ max_image_age_days`** per check; writes `horizon64_image_age_run/1.0` under `.tmp/horizon64-image-age/run.json`. **Not done yet vs. full exit:** **EOL distro** coupling, **cross-registry** mirrors, **warm-cache** pitfalls.

---

### Horizon 65 — **RCA documentation deadline (post-incident SLA)**

**Goal:** **`sev1`**/**`critical`** incidents publish **root-cause analysis** docs within **`max_hours_to_rca_doc`**—pairs **H57** pager routing with **learning-org** accountability.

**Exit criteria**

- **Blameless narrative**, **customer-visible timelines**, **tracked corrective actions**—not only timestamps.

**Implemented in this repository (MVP):** `texts/horizon65_rca_deadline_sample.json` + `scripts/horizon65_rca_deadline_smoke.py` — **`compliant`** iff **severity not in** **`severities_requiring_rca`** **or** **`hours_to_rca_doc ≤ max_hours_to_rca_doc`**; writes `horizon65_rca_deadline_run/1.0` under `.tmp/horizon65-rca-deadline/run.json`. **Not done yet vs. full exit:** **partial RCA** drafts, **legal hold** freezes, **multi-region** coordination clocks.

---

### Horizon 66 — **Deprecated dependency ceiling (tech-debt gate)**

**Goal:** CI/CD surfaces **EOL / deprecated** packages—promotions fail when **`deprecated_dependency_count`** breaches **`max_deprecated_dependencies`** beside **H58** vuln posture.

**Exit criteria**

- **Migration plans**, **compat matrices**, **automated codemods**—not only counters.

**Implemented in this repository (MVP):** `texts/horizon66_deprecated_dep_sample.json` + `scripts/horizon66_deprecated_dep_smoke.py` — **`compliant`** iff **`deprecated_dependency_count ≤ max_deprecated_dependencies`** per check; writes `horizon66_deprecated_dep_run/1.0` under `.tmp/horizon66-deprecated-dep/run.json`. **Not done yet vs. full exit:** **transitive** deprecation graphs, **risk-ranked** waivers, **vendor sunset** calendars.

---

### Horizon 67 — **DSAR export SLA (tiered turnaround)**

**Goal:** **`enterprise`**/**`regulated`** tiers finish **data-export requests** within **`max_hours_to_complete_export`**—pairs **H41** residency/geo discipline with **privacy** operational maturity.

**Exit criteria**

- **Identity assurance**, **partial exports**, **jurisdiction-specific** clocks—not only tier strings.

**Implemented in this repository (MVP):** `texts/horizon67_dsar_export_sample.json` + `scripts/horizon67_dsar_export_smoke.py` — **`compliant`** iff **`customer_tier`** **not in** **`customer_tiers_requiring_fast_export`** **or** **`hours_to_complete_export ≤ max_hours_to_complete_export`**; writes `horizon67_dsar_export_run/1.0` under `.tmp/horizon67-dsar-export/run.json`. **Not done yet vs. full exit:** **DSR portals**, **encryption-at-rest** packaging proofs, **regulator** reporting bridges.

---

### Horizon 68 — **Blocking static-analysis ceiling (quality gate)**

**Goal:** merges fail when **blocking** linter/analyzer findings pile above **`max_blocking_static_findings`**—pairs **H62** branch hygiene with **measurable** code-health bars.

**Exit criteria**

- **Rule packs**, **autofix** bots, **incremental ratchets**—not only counters.

**Implemented in this repository (MVP):** `texts/horizon68_static_block_sample.json` + `scripts/horizon68_static_block_smoke.py` — **`compliant`** iff **`blocking_static_findings ≤ max_blocking_static_findings`** per check; writes `horizon68_static_block_run/1.0` under `.tmp/horizon68-static-block/run.json`. **Not done yet vs. full exit:** **per-language** thresholds, **baseline files**, **severity overrides** with owners.

---

### Horizon 69 — **Vendor API host allow-list (subprocessor egress)**

**Goal:** integrations call only **`allowed_api_hosts`**—hostname-shaped companion to **H42** URL egress lists for **callbacks**, **webhooks**, and **third-party APIs**.

**Exit criteria**

- **Wildcard suffix policies**, **mTLS pinning**, **DNSSEC/DANE** proofs—not only host strings.

**Implemented in this repository (MVP):** `texts/horizon69_vendor_host_sample.json` + `scripts/horizon69_vendor_host_smoke.py` — **`allow`** iff **`requested_host`** (normalized) is in **`allowed_api_hosts`**; writes `horizon69_vendor_host_run/1.0` under `.tmp/horizon69-vendor-host/run.json`. **Not done yet vs. full exit:** **split-horizon** egress, **VPC endpoints**, **regional** mirrors.

---

### Horizon 70 — **Major-incident backlog ceiling (ops hygiene)**

**Goal:** risky changes freeze when **`open_major_incidents`** stacks above **`max_open_major_incidents`**—pairs **H57** paging discipline with **capacity** awareness before promotions.

**Exit criteria**

- **Severity rubrics**, **executive dashboards**, **burn-down drills**—not only integers.

**Implemented in this repository (MVP):** `texts/horizon70_incident_backlog_sample.json` + `scripts/horizon70_incident_backlog_smoke.py` — **`compliant`** iff **`open_major_incidents ≤ max_open_major_incidents`** per check; writes `horizon70_incident_backlog_run/1.0` under `.tmp/horizon70-incident-backlog/run.json`. **Not done yet vs. full exit:** **cross-region** deduping, **customer-visible** status bridges, **SLO-linked** gates.

---

### Horizon 71 — **Payment overdue grace window (subscription continuity)**

**Goal:** **`hours_past_due`** beyond **`max_hours_past_due_allowed`** turns off **service continuity** paths—policy-shaped companion to billing systems beside **Decision gates** economics.

**Exit criteria**

- **Dunning ladders**, **hardship workflows**, **jurisdiction-specific** terms—not only grace hours.

**Implemented in this repository (MVP):** `texts/horizon71_payment_grace_sample.json` + `scripts/horizon71_payment_grace_smoke.py` — **`allow_service`** iff **`hours_past_due ≤ max_hours_past_due_allowed`** per check; writes `horizon71_payment_grace_run/1.0` under `.tmp/horizon71-payment-grace/run.json`. **Not done yet vs. full exit:** **payment processor** webhooks, **partial payments**, **enterprise invoicing** carve-outs.

---

### Horizon 72 — **P1 ticket backlog ceiling (support load)**

**Goal:** risky deploys defer when **`open_p1_tickets`** exceeds **`max_open_p1_tickets`**—pairs **H70** incident posture with **customer-facing** load signals.

**Exit criteria**

- **Severity matrices**, **CSAT bridges**, **on-call rotations** linked to queues—not only counters.

**Implemented in this repository (MVP):** `texts/horizon72_ticket_backlog_sample.json` + `scripts/horizon72_ticket_backlog_smoke.py` — **`compliant`** iff **`open_p1_tickets ≤ max_open_p1_tickets`** per check; writes `horizon72_ticket_backlog_run/1.0` under `.tmp/horizon72-ticket-backlog/run.json`. **Not done yet vs. full exit:** **holiday** staffing models, **multi-product** attribution, **SLA** differentiation by tier.

---

### Horizon 73 — **Contract renewal notice (vendor runway)**

**Goal:** vendor or API contracts need either **`auto_renew`** or at least **`min_notice_days_before_expiry`** runway—procurement hygiene beside **H69** hostname allow-lists.

**Exit criteria**

- **Workflows**, **BAAs**, **procurement committees**—not only booleans and integers.

**Implemented in this repository (MVP):** `texts/horizon73_contract_notice_sample.json` + `scripts/horizon73_contract_notice_smoke.py` — **`compliant`** iff **`auto_renew`** **or** **`days_until_expiry ≥ min_notice_days_before_expiry`**; writes `horizon73_contract_notice_run/1.0` under `.tmp/horizon73-contract-notice/run.json`. **Not done yet vs. full exit:** **multi-year** ramps, **currency** clauses, **termination for convenience** carve-outs.

---

### Horizon 74 — **Penetration-test critical-findings ceiling**

**Goal:** promotions pause while **`open_critical_pen_test_findings`** breaches **`max_open_critical_pen_test_findings`**—structured red-team companion beside **H63** secret scans and **H58** vuln budgets.

**Exit criteria**

- **Scoped engagements**, **retest SLAs**, **bug-bounty** bridges—not only counters.

**Implemented in this repository (MVP):** `texts/horizon74_pentest_findings_sample.json` + `scripts/horizon74_pentest_findings_smoke.py` — **`compliant`** iff **`open_critical_pen_test_findings ≤ max_open_critical_pen_test_findings`** per check; writes `horizon74_pentest_findings_run/1.0` under `.tmp/horizon74-pentest-findings/run.json`. **Not done yet vs. full exit:** **risk-ranked** severities beyond binary critical, **external researcher** coordination.

---

### Horizon 75 — **Disaster-recovery drill recency**

**Goal:** **`months_since_last_drill`** must stay within **`max_months_between_drills`**—operational resilience beside **H54** backup recency and **H65** RCA timelines.

**Exit criteria**

- **Regional game days**, **dependency fault injection**, **customer comms** rehearsals—not only calendar math.

**Implemented in this repository (MVP):** `texts/horizon75_drill_recency_sample.json` + `scripts/horizon75_drill_recency_smoke.py` — **`compliant`** iff **`months_since_last_drill ≤ max_months_between_drills`** per check; writes `horizon75_drill_recency_run/1.0` under `.tmp/horizon75-drill-recency/run.json`. **Not done yet vs. full exit:** **multi-team** rotations, **partial failover** drills, **evidence lockers** for auditors.

---

### Horizon 76 — **Accessibility blocker ceiling**

**Goal:** **`open_a11y_blockers`** must stay within **`max_open_a11y_blockers`**—inclusive UX beside **H58** vuln posture and **H63** hygiene scans.

**Exit criteria**

- **Automated axe-style gates**, **manual audits**, **assistive-tech** smoke—not only counters.

**Implemented in this repository (MVP):** `texts/horizon76_a11y_block_sample.json` + `scripts/horizon76_a11y_block_smoke.py` — **`compliant`** iff **`open_a11y_blockers ≤ max_open_a11y_blockers`** per check; writes `horizon76_a11y_block_run/1.0` under `.tmp/horizon76-a11y-block/run.json`. **Not done yet vs. full exit:** **severity tiers**, **customer-visible flows**, **localization** overlays.

---

### Horizon 77 — **Legal-hold mutation gate**

**Goal:** **`allow_mutate`** is false when **`legal_hold_active`** unless **`counsel_override`**—litigation preservation beside **H21** retention purges and **H56** egress discipline.

**Exit criteria**

- **Custodian workflows**, **chain-of-custody logs**, **e-discovery** bridges—not only booleans.

**Implemented in this repository (MVP):** `texts/horizon77_legal_hold_sample.json` + `scripts/horizon77_legal_hold_smoke.py` — **`allow_mutate`** iff **`¬legal_hold_active ∨ counsel_override`** per check; writes `horizon77_legal_hold_run/1.0` under `.tmp/horizon77-legal-hold/run.json`. **Not done yet vs. full exit:** **matter IDs**, **cross-region** holds, **privileged** tagging.
...
Short list that connects **this** repo to **Horizon 1** and, later, **Horizons 6–77**, without waiting for a “brain” label:

- **Harden data + eval** across more tasks; treat [`further-development-plan.md`](further-development-plan.md) as the **tactical** spine.
- **Know what exists:** H0 (plan), **H1** short-term scripts (handbook), **H2** generative, **H3** memory, **H4** image–text CLIP each have a **local MVP**; **H5** remains lab-only. **H6–H15** cover **composition** through **export** envelopes; **H16–H17** add **semver** contracts and **degradation** tiers; **H18–H19** add **readiness gates** and **audit hash chains**; **H20–H21** add **feature-flag rollout** and **retention purge** smokes; **H22–H23** add **token-bucket** and **blast-radius** smokes; **H24–H25** add **canary gates** and **failover routing** smokes; **H26–H27** add **error budget** and **prompt gate** smokes; **H28–H29** add **idempotency ledger** and **SBOM semver bounds** smokes; **H30–H31** add **lease TTL** and **cardinality budget** smokes; **H32–H33** add **consumer lag** and **purpose matrix** smokes; **H34–H35** add **quorum majority** and **crypto suite** smokes; **H36–H37** add **maintenance freeze** and **pair cardinality** smokes; **H38–H39** add **watermark monotonicity** and **job mutex** smokes; **H40–H41** add **composite policy AND** and **geo-fence residency** smokes; **H42–H43** add **egress URL allow-list** and **credential max-age** smokes; **H44–H45** add **optimistic concurrency revision match** and **payload max-bytes** smokes; **H46–H47** add **latency p99 budget** and **kill-switch global deny** smokes; **H48–H49** add **dual-control distinct approvers** and **pinned digest promote** smokes; **H50–H51** add **wire major-version compat** and **storage quota headroom** smokes; **H52–H53** add **RBAC role subset** and **dry-run mutation gate** smokes; **H54–H55** add **backup recency ceiling** and **sensitive-tier encryption** smokes; **H56–H57** add **TLS version allow-list** and **severity pager routing** smokes; **H58–H59** add **vulnerability budget** and **release-channel signature gate** smokes; **H60–H61** add **SPDX license allow-list** and **maintainer quorum** smokes; **H62–H63** add **protected-branch merge gate** and **secret-scan sweep ceiling** smokes; **H64–H65** add **container image age ceiling** and **RCA deadline** smokes; **H66–H67** add **deprecated dependency ceiling** and **DSAR export SLA** smokes; **H68–H69** add **static-analysis blocking ceiling** and **vendor host allow-list** smokes; **H70–H71** add **major-incident backlog ceiling** and **payment grace window** smokes; **H72–H73** add **P1 ticket backlog ceiling** and **contract renewal notice** smokes; **H74–H75** add **pen-test critical-findings ceiling** and **DR drill recency** smokes; **H76–H77** add **a11y blocker ceiling** and **legal-hold mutation gate** smokes—still **scripts**, not full product.
- **Prototyping lane:** follow [`optional-rd-backlog.md`](optional-rd-backlog.md) for spikes (PEFT, retrieval pooling, etc.).
- **System thinking:** as soon as you add an LLM, invest in **RAG, policies, and logs** in parallel with weights—not after; **H8–H77** add **operational** and **governance** shapes as **tests and contracts**, not only narrative.
