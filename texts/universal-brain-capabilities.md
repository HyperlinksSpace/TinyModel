...
| **Decisions** | `ranked_options`, `decision_matrix`, `build_vs_buy`, `raci`, `cost_benefit`, `recommendation_first`, `go_no_go`, `options_n=3`, `checklist`, `no_checklist` |
| **Visual & structure** | `diagram`, `no_diagram`, `frame_star`, `frame_prep`, `frame_irac`, `swot`, `pestle`, `timeline_chron`, `timeline_reverse` |
| **Planning & editing** | `risks_first`, `benefits_first`, `risks_mitigations`, `revise_draft`, `revise_diff`, `topic_guard`, `topic_must`, `glossary`, `open_questions`, `scenario_cases`, `postmortem`, `five_whys` |
